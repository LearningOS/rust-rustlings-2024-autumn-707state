...
impl<T: Ord + Clone> LinkedList<T> {
    pub fn merge(list_a: LinkedList<T>, list_b: LinkedList<T>) -> Self {
        ...
        while a.is_some() || b.is_some() {
            let node_a = a.map(|ptr| unsafe { &*ptr.as_ptr() });
            let node_b = b.map(|ptr| unsafe { &*ptr.as_ptr() });

            match (node_a, node_b) {
                (Some(a_node), Some(b_node)) => {
                    if a_node.val <= b_node.val {
                        merged_list.add(a_node.val.clone());
                        a = a_node.next; // Move to the next node in list A
                    } else {
                        merged_list.add(b_node.val.clone());
                        b = b_node.next; // Move to the next node in list B
                    }
                }
                (Some(a_node), None) => {
                    merged_list.add(a_node.val.clone());
